...
//Seperate Stuff so i dont have to make a trillion billion "Insert_status_bar_item_here_ListCotrollers".
	...
	for (NSString *theme in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:@"/Library/Application Support/Fufu/" error:nil]) {
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:themePath]) {
			if ([[themePath pathExtension] isEqualToString: @"fufu"]) {
				...
				[specifier setProperty:[NSNumber numberWithInt:66] forKey:@"height"]; //Yucky hack
				[specifier setProperty:[UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"/Library/Application Support/Fufu/%@.fufu/batt/50.png", themeName]] forKey:@"iconImage"];
